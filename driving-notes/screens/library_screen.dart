...
class _LibraryScreenState extends ConsumerState<LibraryScreen> {
  ...
  /// Creates a new (initially empty) collection - lets you set up folders
  /// like "Personal Readings" or "ESV Dramatized" ahead of time, before you
  /// have any files to put in them, without having to go through Import or
  ...
  Future<void> _createCollection() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Created "$name" - it\'ll show up below, empty, until you add files to it.')),
      );
    }
  }
                  ...
                  child: ListView(
                    ...
                    children: _grouped().entries.map((entry) {
                      ...
                      return Card(
                        ...
                        child: ExpansionTile(
                          // Collapsed by default so a library with several
                          // collections isn't one long wall of files to
                          // scroll past - but auto-expanded while actively
                          // searching, since every folder still shown at
                          // that point contains a match you're looking for.
                          // The key (not just initiallyExpanded) changes
                          // when search starts/stops so this actually
                          // re-applies instead of being stuck at whatever
                          // state the tile happened to mount in.
                          key: ValueKey('${collection}_$_searching'),
                          initiallyExpanded: _searching,
                          leading: Icon(
                            collection == 'Personal Readings' ? Icons.mic : Icons.folder,
                            color: Theme.of(context).colorScheme.primary,
                          ),
                          title: Text(
                            collection,
                          ...
                          subtitle: Text(
                            files.isEmpty ? 'Empty' : '${files.length} file${files.length == 1 ? '' : 's'}',
                          ),
                          trailing: files.isEmpty && collection != 'Personal Readings'
                              ? IconButton(
                                  icon: const Icon(Icons.delete_outline),
                                  tooltip: 'Delete this empty folder',
                          ...
                          children: [
                            if (files.isEmpty)
                              Padding(
                                ...
                                child: Text(
                                  'Nothing here yet - import an MP3 or record a reading and choose '
                                  '"$collection" as the collection to put it here.',
