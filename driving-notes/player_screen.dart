...
class _PlayerScreenState extends ConsumerState<PlayerScreen> {
  ...
  Future<void> _generateScriptureCaptions(String bookId, String bookName) async {
    ...
    int? chapterCount;
    try {
      chapterCount = await _bibleText.chapterCount(bookId, 'bsb');
    } catch (_) {
    ...
    try {
      ...
      for (var chapter = startChapter; chapter <= endChapter; chapter++) {
        final chapterVerses = await _bibleText.fetchChapter(bookId, chapter, 'bsb');
      ...
      body: SafeArea(
        ...
        child: SingleChildScrollView(
          ...
          children: [
            ...
            // Round 33: caption box given a guaranteed minimum height of
            // roughly two extra lines on top of its flex share, so more of
            // the passage is on screen at once and the controls below get
            // pushed down rather than the text being clipped to a couple of
            // lines. The flex share also went 3 -> 5 so it keeps most of any
            // spare vertical space too.
            Container(
                width: double.infinity,
                height: 260,
