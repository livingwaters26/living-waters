...
class SettingsService {
  ...
  /// Translation used for MP3 scripture captions.
  Future<String> getCaptionTranslation() async {
    final data = await _readAll();
    final value = data['captionTranslation'] as String?;
    if (value == null || value.trim().isEmpty) return 'bsb';
    return value.trim();
  }

  Future<void> setCaptionTranslation(String id) async {
    final data = await _readAll();
    data['captionTranslation'] = id.trim();
    await _writeAll(data);
  }
}
